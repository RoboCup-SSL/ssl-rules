...
After the yellow card is issued the game will not be stopped, the team will have 10 seconds to move any robot to the mid line of the field, where it can be immediately removed. If a robot is not taken out within time, the game is stopped for <<Robot Substitution, manual substitution>>.

NOTE: In 2020 no penalty will be given to the team that couldn't get the robot out of the field in time, however, in 2021, if the robot gets manually substituted, the ball is placed on the mid line and 1.5 meters away from the teams defense area and the other team gets a free kick.

//before <<Resuming The Game, play resumes>>. The penalized team can choose the robot to remove.
