...
==== Normal Start
.Definition
For multi-stage referee commands, normal start is sent to let the attacking team know that a robot may manipulate the ball. A match cannot be resumed directly via normal start.
...
. When the <<Normal Start, normal start>> command is issued, the attacker is allowed to manipulate the ball. The ball has to only move towards the opponent goal, as measured by its x coordinate in the coordinate system of <<Vision, SSL-Vision>>.
