...
After the yellow card is issued the game will not be stopped, the team will have 10s to move any robot to the mid line of the field, where it can be immediately removed. If a robot is not taken out within time, the game is stopped for <<Robot Substitution, manual substitution>>, the ball is placed 1.5m from the teams defense area and the other team gets a free kick.
