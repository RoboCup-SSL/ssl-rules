...
A robot must not touch the ball while being partially or fully inside the opponent <<Defense Area, defense area>>. This offense increments the <<Fouls, foul counter>> by one, which may result in a yellow card. Additionally, any goals scored within 2 seconds of the end of this offense will not count.
