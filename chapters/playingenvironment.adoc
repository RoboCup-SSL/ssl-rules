...
===== Penalty Mark
The penalty mark defines the point from which a team executes a penalty kick against the opponent goal. It is located on the <<Additional Lines, mid-line>> (not halfway line) and 8 meters (division A) or 6 meters (division B) away from the opponent's goal center.
