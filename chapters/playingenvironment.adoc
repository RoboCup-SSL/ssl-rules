...
These flags are responsible for communicating various intents, such as: <<Timeouts, timeouts>>, <<Emergency stop, emergency stops>>, <<Robot Substitution, manual robot interchange>> and <<Challenge flags, challenges>>.
...
A remote control software or device be provided and replace physical flags.
